...
DATE_COLUMNS = ("fecha", "date", "fecha mercado", "fecha de mercado")
COMMODITY_COLUMNS = ("producto", "grano", "especie", "commodity", "producto/grano")
PRICE_COLUMNS = ("precio", "precio pizarra", "pizarra", "valor", "cotizacion", "cotización")
...
def read_tables(path: Path) -> list[tuple[str, pd.DataFrame]]:
    ...
    return [(sheet, _headered(frame)) for sheet, frame in frames if not _headered(frame).empty]
...
def _default_condition() -> str:
    return "Mercadería disponible, entrega inmediata, pago contado, puesto sobre camión y/o vagón en zona Rosario"
...
def integrate_file(path: Path, integration_date: str) -> list[dict[str, Any]]:
    ...
    for sheet_name, frame in read_tables(path):
        ...
        price_column = _find_column(list(frame.columns), PRICE_COLUMNS)
        if not commodity_column or not price_column:
            continue
        frame = frame.copy()
        ...
        for _, row in frame.iterrows():
            commodity = normalize_commodity(row.get(commodity_column, ""))
            if not commodity:
                continue
            ...
            price_type = normalize_text(row.get(explicit_price_type_column, "")) if explicit_price_type_column else ""
            ...
            output.append({
                "fecha": date_value, "año": date_value[:4] if date_value else "", "mes": date_value[5:7] if date_value else "",
...
def main() -> int:
    ...
    files = sorted(path for path in input_dir.glob("*") if path.is_file() and path.suffix.lower() in TABULAR_EXTENSIONS and not path.name.startswith("~$") and path.name.lower() != "desktop.ini" and path.stat().st_size > 0)
